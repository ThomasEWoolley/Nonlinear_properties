...
sty={'-','--',':','-.'}
for i=length(G2p):-1:1
    surf([G2p(i)+0*G3;G2p(i)+0*G3],[G3;G3],5+[G3;G3],'facecol','no','edgecol','interp','linew',2,'linestyle',sty{i});
end
G2x=linspace(1,4.9);
G3y=[-2 1.5]
% for i=1:length(G3y)
...
% export_fig('../Pictures/DG2G3_vert.png','-r300')
% 
%%
close all
f=figure;
...
for i=4:-1:1%1:2:length(G2p)
    p2_eval=p2v(G2p(i),G3);
    p3_eval=p3v(G2p(i),G3);

    surf([p2_eval;p2_eval],[p3_eval;p3_eval],5+[G3;G3],'facecol','no',...
        'edgecol','interp',...
        'linew',2,'linestyle',sty{i});

end

% export_fig(f,'../Pictures/p2p3_vert.png','-r300')
