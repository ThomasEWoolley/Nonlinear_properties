...
% c2s=convert2shell(logical(regiona2));

% S=[sum(region1(Ids)),sum(region2(Ids)),sum(region3(Ids)),sum(region4(Ids)),sum(region5(Ids))]
% sum(S)
% sum(Ids(:))
...
trans = 1;
for i=1:5
h{i}=scatter3(D(Ids),G2(Ids),G3(Ids),1,eval(['region',num2str(i),'(Ids)'])+i,'filled');
...
export_fig('../Pictures/DG2G3.png','-r300')

%%
% ech=extractCol(h);
% cols=UniqueRGB(extractCol(h));
% cols=cols([1 3 4 5 2],:);
% displayRGBColours(cols);
% export_fig('Cols_rgb.png','-r300')
% save('RGB_cols.mat','cols')
